...
const Logger = (function() {
     ...
     * @returns {string} - Email del usuario o 'system'
     */
    function getCurrentUserEmail() {
        const session = getSession();
        if (session) {
            return session.userEmail || session.email || 'system';
        }
        return 'system';
    }
    ...
    async function audit(actionType, module, entityId, field = '', oldValue = '', newValue = '', details = '') {
        ...
        const args = [
          session.userEmail || session.email,      // userEmail del usuario (priorizar userEmail)
    ...
    async function error(module, action, errorMsg, level = 'ERROR') {
        const session = getSession();
        const email = session ? (session.userEmail || session.email) : 'unknown';
    ...
    */
    async function woApproved(woId, approvedBy) {
        const session = getSession();
        const userEmail = session ? (session.userEmail || session.email) : 'system';
